    ...
    <div v-if="saved" class="list-group-item">
        <div v-show="editable">
            <div class="form-group row">
                <label v-bind:for="'resource-link-url' + datasetField.id" class="col-sm-2 col-form-label">URL</label>
                <div class="col-sm-10">
                    <input v-model="data.url" v-bind:id="'resource-link-url' + datasetField.id" placeholder="URL" class="form-control" />
                </div>
            </div>
            <div class="form-group row">
                <label v-bind:for="'resource-link-description' + datasetField.id" class="col-sm-2 col-form-label">Beskrivelse</label>
                <div class="col-sm-10">
                    <input v-model="data.description" v-bind:id="'resource-link-description' + datasetField.id" placeholder="Beskrivelse" class="form-control" />
                </div>
            </div>
        ...
        <div v-show="!editable" v-bind:class="{'d-flex justify-content-between align-items-center': !editable}">
            <a v-bind:href="datasetField.url">{{ datasetField.description}}</a>
...
<style src="./DatasetField.scss"></style>
