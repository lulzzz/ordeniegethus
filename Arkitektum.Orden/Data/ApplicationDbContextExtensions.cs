...
namespace Arkitektum.Orden.Data
{
    public static class ApplicationDbContextExtensions
    {
        ...
        public static void EnsureSeedData(this ApplicationDbContext context, IApplicationBuilder app)
        {
            if (!context.Database.GetPendingMigrations().Any())
            {
                ...
                if (!context.Users.Any())
                {
                    ...
                    {
                        UserName = "admin",
                        Email = "dev@arkitektum.no",

                    };
                    IdentityResult result = userManager.CreateAsync(adminUser, "password_goes_here").Result;

                    if (result.Succeeded)
                        userManager.AddToRoleAsync(adminUser, Roles.Admin);
                }
